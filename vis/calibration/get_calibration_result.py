...
print(np.array(rvecs), np.array(tvecs)*0.01)
cv.destroyAllWindows()


from scipy.spatial.transform import Rotation as R
camera = [0,0,0,0,0,0]
camera[0:3] = np.array([0.05189377, 0.02226017, 0.24208615])
camera[3:6] = R.from_rotvec(np.array([-0.65143779, 0.86070041, 1.43764448])).as_euler("xyz")*180/np.pi
...
from src.ur5 import get_current_tcp_in_rpy
hand = get_current_tcp_in_rpy()
hand[3:6]*= 180/np.pi
print(hand)

hand = hand.tolist()+hand.tolist()+hand.tolist()
camera = camera+camera+camera
# camera = camera.tolist()+camera.tolist()+camera.tolist()
